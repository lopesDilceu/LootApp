...
class RawgApiProvider extends GetConnect {
  ...
  final String _apiKey = "f993c9ed681743a6b0e4368d82a55949"; // Guarde de forma segura
  ...
  Future<List<String>> getGameScreenshots(int rawgGameId) async {
    ...
    try {
      final response = await get(endpoint, query: {'key': _apiKey}); //httpClient.get
      if (response.isOk && response.body['results'] != null) {
        ...
        print("[RawgApiProvider] Erro ao buscar screenshots RAWG: ${response.statusCode} - ${response.statusText}");
      }
    } catch (e) {
  ...
  Future<int?> findRawgGameId({String? steamAppId, String? title}) async {
    ...
    try {
      ...
      if (response.isOk && response.body['results'] != null && (response.body['results'] as List).isNotEmpty) {
        ...
        for(var gameData in games) {
            if (steamAppId != null && gameData['stores'] != null) {
                for (var storeEntry in gameData['stores']) {
                    if (storeEntry['store'] != null && storeEntry['store']['slug'] == 'steam' && 
                        gameData['id'] != null /* e comparar o ID da URL da loja com steamAppId */ ) {
    ...
    try {
      final response = await get("/games", query: {'key': _apiKey, 'search': title, 'page_size': '1'});
      if (response.isOk && response.body['results'] != null && (response.body['results'] as List).isNotEmpty) {
        return (response.body['results'][0]['id'] as int?);
      }
      print("[RawgApiProvider] Jogo não encontrado na RAWG por título '${title}': ${response.statusCode}");
    } catch (e) {
      print("[RawgApiProvider] Exceção ao encontrar jogo na RAWG por título: $e");
    }
    return null;
  }
   Future<int?> findRawgGameIdBySteamId(String steamAppId) async {
    ...
    print("[RawgApiProvider] findRawgGameIdBySteamId não implementado de forma otimizada. Usando título do deal como fallback se disponível.");
